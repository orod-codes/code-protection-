...
# C++ only: lists sibling headers in the .cpp directory; their contents are hashed with the .cpp.
CPP_HDR_MANIFEST_MARKER = "__EI_HDRS__:"
# Python / JS / Java / C#: other source files in the same directory are hashed with this file.
SIBLING_MANIFEST_MARKER = "__EI_SIB__:"
REGISTRY_FILE = ".protection_registry.json"

_SIBLING_INTEGRITY_EXTS: Dict[str, Tuple[str, ...]] = {
    ".py": (".py",),
    ".js": (".js", ".mjs", ".cjs"),
    ".java": (".java",),
    ".cs": (".cs",),
...
def _python_runtime_guard_block(sib_line: str) -> str:
    """
    Runtime self-verification block embedded in generated Python files.
    """
    return f"""# --- BEGIN ENGINE RUNTIME GUARD ---
{sib_line}
...
def _ei_verify_self() -> None:
    ...
    for _ei_line in _ei_lines:
        _ei_stripped = _ei_line.strip()
        if _ei_inj in _ei_stripped or _ei_lock in _ei_stripped or _ei_rec in _ei_stripped:
            continue
        if "__EI_SIB__:" in _ei_stripped and _ei_stripped.startswith("#"):
            continue
        _ei_current_lines.append(_ei_line)
    _ei_sib_names = []
    for _ei_line in _ei_lines:
        _ei_st = _ei_line.strip()
        if "__EI_SIB__:" in _ei_st and _ei_st.startswith("#"):
            _ei_i = _ei_st.find("__EI_SIB__:")
            _ei_rest = _ei_st[_ei_i + len("__EI_SIB__:") :].strip()
            if _ei_rest:
                _ei_sib_names = _ei_rest.split()
            break
    _ei_blob_x = ""
    if _ei_sib_names:
        _ei_d = _ei_os.path.dirname(_ei_os.path.abspath(__file__))
        _ei_pb = []
        for _ei_sn in _ei_sib_names:
            _ei_pp = _ei_os.path.join(_ei_d, _ei_sn)
            try:
                with open(_ei_pp, "r", encoding="utf-8") as _ei_sf:
                    _ei_pb.append("EI_FN:" + _ei_sn + "\\n" + _ei_sf.read())
            except Exception:
                _ei_pb.append("EI_FN:" + _ei_sn + "\\n")
        _ei_blob_x = "\\n".join(_ei_pb)
    _ei_current_code = "\\n".join(_ei_current_lines)
    if _ei_blob_x:
        _ei_current_code = _ei_current_code + "\\n" + _ei_blob_x
...
def _inject_python_runtime_guard(content: str, source_abs: str) -> str:
    """
    Inject runtime guard in Python source before hash markers are added.
    """
    if "# --- BEGIN ENGINE RUNTIME GUARD ---" in content:
        return content
    names = _sibling_basenames_for_integrity(source_abs) if source_abs else []
    sib_line = "# " + SIBLING_MANIFEST_MARKER + (" " + " ".join(names) if names else "")
    guard = _python_runtime_guard_block(sib_line)
...
def _javascript_runtime_guard_block(sib_line: str) -> str:
    """
...
function _ei_verify_self() {
  ...
  for (const _ei_line of _ei_lines) {
    const _ei_stripped = _ei_line.trim();
    if (_ei_stripped.includes(_ei_inj) || _ei_stripped.includes(_ei_lock) || _ei_stripped.includes(_ei_rec)) {
      continue;
    }
    if (_ei_stripped.includes('__EI_SIB__:') && _ei_stripped.startsWith('//')) {
      continue;
    }
  ...
  let _ei_sib_names = [];
  for (const _ei_line of _ei_lines) {
    const _ei_st = _ei_line.trim();
    if (_ei_st.includes('__EI_SIB__:') && _ei_st.startsWith('//')) {
      const _ei_i = _ei_st.indexOf('__EI_SIB__:');
      const _ei_rest = _ei_st.slice(_ei_i + '__EI_SIB__:'.length).trim();
      if (_ei_rest) _ei_sib_names = _ei_rest.split(/\\s+/);
      break;
    }
  }
  let _ei_blob_x = '';
  if (_ei_sib_names.length) {
    const _ei_d = _ei_path.dirname(_ei_path.resolve(__filename));
    const _ei_pb = [];
    for (const _ei_sn of _ei_sib_names) {
      const _ei_pp = _ei_path.join(_ei_d, _ei_sn);
      try {
        _ei_pb.push('EI_FN:' + _ei_sn + '\\n' + _ei_fs.readFileSync(_ei_pp, 'utf8'));
      } catch (_e) {
        _ei_pb.push('EI_FN:' + _ei_sn + '\\n');
      }
    }
    _ei_blob_x = _ei_pb.join('\\n');
  }
  let _ei_current_code = _ei_current_lines.join('\\n');
  if (_ei_blob_x) _ei_current_code = _ei_current_code + '\\n' + _ei_blob_x;
...
def _inject_javascript_runtime_guard(content: str, source_abs: str) -> str:
    """
    Inject runtime guard in JavaScript source before hash markers are added.
    """
    names = _sibling_basenames_for_integrity(source_abs) if source_abs else []
    sib_line = "// " + SIBLING_MANIFEST_MARKER + (" " + " ".join(names) if names else "")
    guard = _javascript_runtime_guard_block(sib_line)
...
def _java_runtime_guard_block(embedded_path: str, sib_line: str) -> str:
    rp = embedded_path.replace("\\", "\\\\").replace('"', '\\"')
    return f"""// --- BEGIN ENGINE RUNTIME GUARD ---
{sib_line}
final class _eiGuard {{
    private static final String _rel = "{rp}";
    static void run() {{
        try {{
            java.nio.file.Path p = java.nio.file.Paths.get(System.getProperty("user.dir")).resolve(_rel).normalize();
            byte[] raw = java.nio.file.Files.readAllBytes(p);
            String text = new String(raw, java.nio.charset.StandardCharsets.UTF_8);
            String[] lines = text.split("\\n", -1);
            final String mk = "__HASH_INJECTED__" + ":";
            final String lk = "__LOCKED__" + ":";
            final String rk = "__INJECTION_RECORD__" + ":";
            java.util.List<String> sibs = new java.util.ArrayList<>();
            for (String line : lines) {{
                String t = line.trim();
                if (t.startsWith("//") && t.contains("__EI_SIB__:")) {{
                    int i = t.indexOf("__EI_SIB__:");
                    String rest = t.substring(i + "__EI_SIB__:".length()).trim();
                    if (!rest.isEmpty()) {{
                        for (String w : rest.split("\\\\s+")) sibs.add(w);
                    }}
                    break;
                }}
            }}
            ...
            for (String line : lines) {{
                ...
                if (t.contains(mk)) {{
                    int i = t.indexOf(mk);
                    String tail = t.substring(i + mk.length()).trim();
                    String[] parts = tail.split("\\\\s+");
                    if (parts.length >= 2) stored = parts[1];
                    break;
                }}
            }}
            if (stored == null) {{
                System.err.println("EXECUTION BLOCKED: missing hash marker");
                System.exit(1);
            }}
            StringBuilder cur = new StringBuilder();
            for (String line : lines) {{
                String t = line.trim();
                if (t.contains(mk) || t.contains(lk) || t.contains(rk)) continue;
                if (t.startsWith("//") && t.contains("__EI_SIB__:")) continue;
                if (cur.length() > 0) cur.append('\\n');
                cur.append(line);
            }}
            String payload = cur.toString();
            if (!sibs.isEmpty()) {{
                java.nio.file.Path dir = p.getParent();
                StringBuilder bx = new StringBuilder();
                for (String bn : sibs) {{
                    if (bx.length() > 0) bx.append('\\n');
                    bx.append("EI_FN:").append(bn).append('\\n');
                    try {{
                        bx.append(java.nio.file.Files.readString(dir.resolve(bn), java.nio.charset.StandardCharsets.UTF_8));
            ...
            for (byte b : dig) hx.append(String.format("%02x", b));
            if (!hx.toString().equals(stored)) {{
                System.err.println("EXECUTION BLOCKED: code was modified");
                System.exit(1);
            }}
        }} catch (Throwable e) {{
            System.err.println("EXECUTION BLOCKED");
...
def _inject_java_runtime_guard(content: str, embedded_path: str, source_abs: str) -> str:
    if "// --- BEGIN ENGINE RUNTIME GUARD ---" in content:
        return content
    # Customer files may be named *.customer.java; filename must not require matching public class.
    content, _n_pub = re.subn(r"\bpublic\s+class\s+(\w+)\b", r"class \1", content, count=1)
    names = _sibling_basenames_for_integrity(source_abs) if source_abs else []
    sib_line = "// " + SIBLING_MANIFEST_MARKER + (" " + " ".join(names) if names else "")
    guard = _java_runtime_guard_block(embedded_path, sib_line)
    static_blk = (
        "    static {\n"
        "        try { _eiGuard.run(); } catch (Throwable t) {\n"
        "            System.err.println(\"EXECUTION BLOCKED\");\n"
        "            System.exit(1);\n"
        "        }\n"
        "    }\n"
...
def _csharp_runtime_guard_block(embedded_path: str, sib_line: str) -> str:
    ep = embedded_path.replace("\\", "\\\\").replace('"', '\\"')
    return f"""// --- BEGIN ENGINE RUNTIME GUARD ---
{sib_line}
using System.IO;
using System.Linq;
...
static class _eiIntegrity
{{
    private const string _rel = "{ep}";

    internal static void Verify()
    {{
        try
        {{
            var path = Path.GetFullPath(Path.Combine(Directory.GetCurrentDirectory(), _rel));
            var text = File.ReadAllText(path);
            var lines = text.Split('\\n');
            const string mk = "__HASH_INJECTED__" + ":";
            const string lk = "__LOCKED__" + ":";
            const string rk = "__INJECTION_RECORD__" + ":";
            var sibs = new System.Collections.Generic.List<string>();
            foreach (var line in lines)
            {{
                var t = line.Trim();
                if (t.StartsWith("//") && t.Contains("__EI_SIB__:"))
                {{
                    var i = t.IndexOf("__EI_SIB__:", StringComparison.Ordinal);
                    var rest = t.Substring(i + "__EI_SIB__:".Length).Trim();
                    if (rest.Length > 0)
                        sibs.AddRange(rest.Split((char[])null, StringSplitOptions.RemoveEmptyEntries));
                    break;
                }}
            }}
            string stored = "";
            ...
            {{
                ...
                if (t.Contains(mk))
                {{
                    var idx = t.IndexOf(mk, StringComparison.Ordinal);
                    var tail = t.Substring(idx + mk.Length).Trim();
                    var parts = tail.Split((char[])null, StringSplitOptions.RemoveEmptyEntries);
                    if (parts.Length >= 2) stored = parts[1];
                    break;
                }}
            }}
            ...
            {{
                Console.Error.WriteLine("EXECUTION BLOCKED: missing hash marker");
                Environment.Exit(1);
            }}
            var kept = lines.Where(line =>
            {{
                var t = line.Trim();
                if (t.Contains(mk) || t.Contains(lk) || t.Contains(rk)) return false;
                if (t.StartsWith("//") && t.Contains("__EI_SIB__:")) return false;
                return true;
            }});
            var cur = string.Join("\\n", kept);
            if (sibs.Count > 0)
            {{
                ...
                foreach (var bn in sibs)
                {{
                    if (bx.Length > 0) bx.Append('\\n');
                    bx.Append("EI_FN:").Append(bn).Append('\\n');
                    try {{ bx.Append(File.ReadAllText(Path.Combine(dir, bn))); }} catch {{ }}
                }}
                cur = cur + "\\n" + bx.ToString();
            }}
            using (var sha = SHA256.Create())
            {{
                var hash = sha.ComputeHash(Encoding.UTF8.GetBytes(cur));
                var hx = BitConverter.ToString(hash).Replace("-", "").ToLowerInvariant();
                if (hx != stored)
                {{
                    Console.Error.WriteLine("EXECUTION BLOCKED: code was modified");
                    Environment.Exit(1);
                }}
            }}
        }}
        ...
        {{
            Console.Error.WriteLine("EXECUTION BLOCKED");
            Environment.Exit(1);
        }}
    }}
}}
...
def _inject_csharp_runtime_guard(content: str, embedded_path: str, source_abs: str) -> str:
    if "// --- BEGIN ENGINE RUNTIME GUARD ---" in content:
        return content
    names = _sibling_basenames_for_integrity(source_abs) if source_abs else []
    sib_line = "// " + SIBLING_MANIFEST_MARKER + (" " + " ".join(names) if names else "")
    guard = _csharp_runtime_guard_block(embedded_path, sib_line)
    lines = content.split("\n")
    insert_at = 0
    for i, line in enumerate(lines):
        stripped = line.strip()
        if stripped.startswith("using ") and stripped.endswith(";"):
            insert_at = i + 1
        elif stripped and not stripped.startswith("using "):
            break
    lines.insert(insert_at, guard)
    content = "\n".join(lines)
    new_content, n = re.subn(
        r"(class\s+Program\s*\{)",
        r"\1\n    static Program() { _eiIntegrity.Verify(); }\n",
        content,
        count=1,
        flags=re.IGNORECASE,
    )
...
def _cpp_sibling_header_basenames(cpp_path: str) -> List[str]:
    """Headers in the same directory as the .cpp (not recursive)."""
    d = os.path.dirname(os.path.abspath(cpp_path))
...
def _cpp_parse_hdr_manifest(text: str) -> List[str]:
    """Parse // __EI_HDRS__: a.h b.h from injected source."""
    token = CPP_HDR_MANIFEST_MARKER
    for line in text.split("\n"):
        st = line.strip()
        if not st.startswith("//") or token not in st:
            continue
        i = st.find(token)
        rest = st[i + len(token) :].strip()
        if not rest:
            return []
        return rest.split()
    return []
...
def _cpp_header_blob(cpp_dir: str, basenames: List[str]) -> str:
    """Canonical blob appended to .cpp text for SHA-256 (must match C++ runtime)."""
    if not basenames:
        return ""
    parts: List[str] = []
    for bn in basenames:
        p = os.path.join(cpp_dir, bn)
        try:
            with open(p, "r", encoding="utf-8", errors="replace") as f:
                t = f.read()
        except OSError:
            t = ""
        parts.append("//__EI_PART__:" + bn + "\n" + t)
    return "\n".join(parts)


def _cpp_payload_for_hash(content: str, cpp_abs_path: str) -> str:
    """
    Integrity payload for C++: .cpp text without hash/lock/record/manifest lines,
    plus header bodies listed on // __EI_HDRS__: (same directory). If there is no
    __EI_HDRS__ line (legacy injection), behavior matches the old single-file hash.
    """
    has_manifest = "__EI_HDRS__:" in content
    lines = content.split("\n")
    kept: List[str] = []
    for line in lines:
        st = line.strip()
        if INJECTION_MARKER in st or LOCK_MARKER in st or INJECTION_RECORD_MARKER in st:
            continue
        if st.startswith("//") and CPP_HDR_MANIFEST_MARKER in st:
            continue
        kept.append(line)
    base = "\n".join(kept)
    if not has_manifest:
        return base
    cpp_dir = os.path.dirname(os.path.abspath(cpp_abs_path))
    hdrs = _cpp_parse_hdr_manifest(content)
    blob = _cpp_header_blob(cpp_dir, hdrs)
    return base + ("\n" + blob if blob else "")


def _cpp_hdr_manifest_line(cpp_abs_path: str) -> str:
    hdrs = _cpp_sibling_header_basenames(cpp_abs_path)
    return "// " + CPP_HDR_MANIFEST_MARKER + (" " + " ".join(hdrs) if hdrs else "")


def _cpp_runtime_guard_block(manifest_line: str) -> str:
    # manifest_line records which headers were bundled at inject time (contents hashed).
    ml = manifest_line
    return f"""// --- BEGIN ENGINE RUNTIME GUARD ---
{ml}
#include <fstream>
...
namespace _ei_ns {{
static std::string _ei_read_all(const char* path) {{
...
static std::vector<std::string> _ei_split_lines(const std::string& s) {{
...
static bool _ei_trim_has(const std::string& line, const char* sub) {{
    size_t x = 0, y = line.size();
    while (x < y && (line[x] == ' ' || line[x] == '\\t' || line[x] == '\\r')) x++;
    while (y > x && (line[y-1] == ' ' || line[y-1] == '\\t' || line[y-1] == '\\r')) y--;
    std::string t = line.substr(x, y - x);
    return t.find(sub) != std::string::npos;
}}
static std::string _ei_dirname(const char* path) {{
    std::string s(path);
    size_t n = s.find_last_of("/\\\\");
    if (n == std::string::npos) return ".";
    return s.substr(0, n);
}}
static std::string _ei_join(const std::string& dir, const std::string& name) {{
    if (dir.empty()) return name;
    char c = dir.back();
    if (c == '/' || c == '\\\\') return dir + name;
    return dir + "/" + name;
}}
static std::vector<std::string> _ei_parse_hdrs(const std::vector<std::string>& lines) {{
    const char* tag = "// __EI_HDRS__:";
    for (const auto& line : lines) {{
        if (!_ei_trim_has(line, tag)) continue;
        std::string t = line;
        size_t p = t.find(tag);
        if (p == std::string::npos) continue;
        std::string rest = t.substr(p + std::strlen(tag));
        while (!rest.empty() && (rest[0] == ' ' || rest[0] == '\\t')) rest.erase(0, 1);
        std::vector<std::string> names;
        std::istringstream iss(rest);
        std::string w;
        while (iss >> w) names.push_back(w);
        return names;
    }}
    return {{}};
}}
static void _ei_verify_file(const char* path) {{
    const char* mk = "// " "__HASH_INJECTED__" ":";
    const char* lk = "// " "__LOCKED__" ":";
    const char* rk = "// " "__INJECTION_RECORD__" ":";
    const char* hm = "// __EI_HDRS__:";
    std::string text = _ei_read_all(path);
    if (text.empty()) {{
        std::cerr << "EXECUTION BLOCKED: cannot read source file\\n";
        std::exit(1);
    }}
    auto lines = _ei_split_lines(text);
    std::vector<std::string> hdrs = _ei_parse_hdrs(lines);
    std::string stored;
    for (const auto& line : lines) {{
        if (_ei_trim_has(line, mk)) {{
            std::string t = line;
            size_t p = t.find(mk);
            if (p != std::string::npos) {{
                std::istringstream iss(t.substr(p + std::strlen(mk)));
                std::string inj, codeh;
                iss >> inj >> codeh;
                stored = codeh;
            }}
            break;
    ...
    if (stored.empty()) {{
        std::cerr << "EXECUTION BLOCKED: missing hash marker\\n";
        std::exit(1);
    }}
    std::string cur;
    for (const auto& line : lines) {{
        if (_ei_trim_has(line, mk) || _ei_trim_has(line, lk) || _ei_trim_has(line, rk)) continue;
        if (_ei_trim_has(line, hm)) continue;
        if (!cur.empty()) cur.push_back('\\n');
        cur += line;
    }}
    std::string blob;
    std::string dir = _ei_dirname(path);
    for (const auto& bn : hdrs) {{
        std::string p = _ei_join(dir, bn);
        std::string body = _ei_read_all(p.c_str());
    ...
    std::string payload = cur;
    if (!blob.empty()) {{
        payload.push_back('\\n');
        payload += blob;
    }}
    unsigned char h[SHA256_DIGEST_LENGTH];
    SHA256(reinterpret_cast<const unsigned char*>(payload.data()), payload.size(), h);
    char hx[65];
    for (int i = 0; i < 32; i++) std::sprintf(hx + i * 2, "%02x", h[i]);
    hx[64] = 0;
    if (stored != std::string(hx)) {{
        std::cerr << "EXECUTION BLOCKED: code was modified\\n";
        std::exit(1);
    }}
}}
struct _ei_run {{
    _ei_run() {{ _ei_verify_file(__FILE__); }}
}};
static _ei_run _ei_guard_instance;
}}
// --- END ENGINE RUNTIME GUARD ---
"""


def _inject_cpp_runtime_guard(content: str, cpp_abs_path: str) -> str:
    if "// --- BEGIN ENGINE RUNTIME GUARD ---" in content:
        return content
    manifest = _cpp_hdr_manifest_line(cpp_abs_path)
    return _cpp_runtime_guard_block(manifest) + "\n" + content


def _line_is_sibling_manifest(line: str) -> bool:
    st = line.strip()
    return SIBLING_MANIFEST_MARKER in st and (st.startswith("#") or st.startswith("//"))


def _parse_sibling_manifest(text: str) -> List[str]:
    token = SIBLING_MANIFEST_MARKER
    for line in text.split("\n"):
        st = line.strip()
        if token not in st or not (st.startswith("#") or st.startswith("//")):
            continue
        i = st.find(token)
        rest = st[i + len(token) :].strip()
        if not rest:
...
def _sibling_basenames_for_integrity(file_abs: str) -> List[str]:
    ext = os.path.splitext(file_abs)[1].lower()
    exts = _SIBLING_INTEGRITY_EXTS.get(ext)
    if not exts:
        return []
    d = os.path.dirname(os.path.abspath(file_abs))
    self_bn = os.path.basename(file_abs)
    out: List[str] = []
    try:
        for name in os.listdir(d):
            le = os.path.splitext(name)[1].lower()
            if le in exts and name != self_bn:
                out.append(name)
    except OSError:
        return []
    out.sort(key=lambda x: x.lower())
...
def _sibling_integrity_blob(dir_path: str, basenames: List[str]) -> str:
    if not basenames:
        return ""
    parts: List[str] = []
    for bn in basenames:
        p = os.path.join(dir_path, bn)
        try:
            with open(p, "r", encoding="utf-8", errors="replace") as f:
                body = f.read()
        except OSError:
            body = ""
        parts.append("EI_FN:" + bn + "\n" + body)
    return "\n".join(parts)


def _sibling_payload_for_hash(content: str, file_abs: str) -> str:
    """Strip engine + sibling-manifest lines; append sibling file blob when manifest is present."""
    ext = os.path.splitext(file_abs)[1].lower()
    if ext not in _SIBLING_INTEGRITY_EXTS:
        return content
    lines = content.split("\n")
    kept: List[str] = []
    for line in lines:
        ...
        if INJECTION_MARKER in st or LOCK_MARKER in st or INJECTION_RECORD_MARKER in st:
            continue
        if _line_is_sibling_manifest(line):
            continue
        kept.append(line)
    base = "\n".join(kept)
    if SIBLING_MANIFEST_MARKER not in content:
        return base
    names = _parse_sibling_manifest(content)
    blob = _sibling_integrity_blob(os.path.dirname(os.path.abspath(file_abs)), names)
    return base + ("\n" + blob if blob else "")


def _c_runtime_guard_block() -> str:
    return r"""// --- BEGIN ENGINE RUNTIME GUARD ---
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <openssl/sha.h>

static int _ei_has(const char* ln, const char* sub) {
    while (*ln == ' ' || *ln == '\t' || *ln == '\r') ln++;
    return strstr(ln, sub) != NULL;
}

static void _ei_verify_c(const char* path) {
    const char* mk = "// " "__HASH_INJECTED__" ":";
    const char* lk = "// " "__LOCKED__" ":";
    const char* rk = "// " "__INJECTION_RECORD__" ":";
    FILE* fp = fopen(path, "rb");
    if (!fp) { fprintf(stderr, "EXECUTION BLOCKED\n"); exit(1); }
    fseek(fp, 0, SEEK_END);
    long sz = ftell(fp);
    fseek(fp, 0, SEEK_SET);
    ...
    while (p) {
        ...
        if (_ei_has(p, mk)) {
            ...
            if (q) {
                q += strlen(mk);
                char inj[96], codeh[96];
                if (sscanf(q, " %95s %95s", inj, codeh) >= 2)
                    strncpy(stored, codeh, sizeof(stored) - 1);
            }
        }
        if (!nl) break;
        *nl = '\n';
    ...
    if (stored[0] == '\0') { free(buf); fprintf(stderr, "EXECUTION BLOCKED\n"); exit(1); }
    size_t cl = 0;
    char* cur = (char*)malloc((size_t)sz + 2);
    if (!cur) { free(buf); exit(1); }
    p = buf;
    while (p) {
        char* nl = strchr(p, '\n');
        if (nl) *nl = 0;
        if (!_ei_has(p, mk) && !_ei_has(p, lk) && !_ei_has(p, rk)) {
            size_t L = strlen(p);
            if (cl > 0) cur[cl++] = '\n';
            memcpy(cur + cl, p, L);
            cl += L;
    ...
    cur[cl] = 0;
    unsigned char h[SHA256_DIGEST_LENGTH];
    SHA256((unsigned char*)cur, cl, h);
    char hx[65];
    int i;
    for (i = 0; i < 32; i++) sprintf(hx + i * 2, "%02x", h[i]);
    hx[64] = 0;
    free(cur);
    free(buf);
    if (strcmp(stored, hx) != 0) { fprintf(stderr, "EXECUTION BLOCKED\n"); exit(1); }
}

static void _ei_ctor(void) __attribute__((constructor));
static void _ei_ctor(void) { _ei_verify_c(__FILE__); }
// --- END ENGINE RUNTIME GUARD ---
"""


def _inject_c_runtime_guard(content: str) -> str:
    if "// --- BEGIN ENGINE RUNTIME GUARD ---" in content:
        return content
    return _c_runtime_guard_block() + "\n" + content


def _apply_runtime_guards(
    source_ext: str,
    content: str,
    java_cs_embedded_path: str,
    source_abs_path: Optional[str] = None,
) -> str:
    ext = source_ext.lower()
    src = source_abs_path or ""
    if ext == ".py":
        return _inject_python_runtime_guard(content, src)
    if ext == ".js":
        return _inject_javascript_runtime_guard(content, src)
    if ext == ".java":
        return _inject_java_runtime_guard(content, java_cs_embedded_path, src)
    if ext == ".cs":
        return _inject_csharp_runtime_guard(content, java_cs_embedded_path, src)
    if ext in (".cpp", ".cc", ".cxx"):
        return _inject_cpp_runtime_guard(content, source_abs_path or "")
    if ext == ".c":
        return _inject_c_runtime_guard(content)
    return content
...
def _build_injected_content(file_path: str, content: str) -> Tuple[str, str, str, str]:
    ...
    ext = os.path.splitext(file_path)[1].lower()
    if ext in (".cpp", ".cc", ".cxx"):
        hash_input = _cpp_payload_for_hash(content, file_path)
    elif ext in _SIBLING_INTEGRITY_EXTS:
        hash_input = _sibling_payload_for_hash(content, file_path)
    else:
        hash_input = content
    code_hash = hashlib.sha256(hash_input.encode("utf-8")).hexdigest()
...
def generate_injected_file(
    source_path: str,
    output_path: Optional[str] = None,
    customer_root: Optional[str] = None,
) -> Tuple[bool, str]:
    """
    ...
    Args:
        ...
        customer_root: If set (e.g. copied customer folder), Java/C# embed path relative
            to this root; run with cwd = that folder.
    ...
    if customer_root:
        java_cs_path = os.path.relpath(
            source_abs, os.path.abspath(customer_root)
        ).replace("\\", "/")
    else:
        java_cs_path = os.path.basename(source_abs)

    # Embed startup self-verification for supported runtimes (Python, JS, Java, C#, C/C++).
    content = _apply_runtime_guards(
        source_ext, content, java_cs_path, source_abs
    )
...
def generate_customer_folder(source_folder: str, output_folder: Optional[str] = None) -> Tuple[bool, Dict[str, str]]:
    ...
    for file_path in source_files:
        ...
        root_no_ext, _ = os.path.splitext(file_path)
        temp_output = root_no_ext + ".ei_build" + file_ext
        success, message = generate_injected_file(
            file_path, temp_output, customer_root=output_abs
        )
...
def inject_hash(file_path: str) -> bool:
    ...
    inj_ext = os.path.splitext(abs_path)[1]
    content = _apply_runtime_guards(
        inj_ext, content, os.path.basename(abs_path), abs_path
    )
...
def verify_code_integrity(file_path: str) -> Tuple[bool, str]:
    ...
    ext_chk = os.path.splitext(abs_path)[1].lower()
    if ext_chk in (".cpp", ".cc", ".cxx"):
        # C++: same payload as runtime guard (.cpp minus markers + bundled headers)
        current_code_content = _cpp_payload_for_hash(content, abs_path)
        current_hash = hashlib.sha256(current_code_content.encode("utf-8")).hexdigest()
    elif ext_chk in _SIBLING_INTEGRITY_EXTS:
        current_code_content = _sibling_payload_for_hash(content, abs_path)
        current_hash = hashlib.sha256(current_code_content.encode("utf-8")).hexdigest()
    else:
        current_code_lines = []
        for line in lines:
